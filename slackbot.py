...
SLACK_TOKEN = "xoxb-1687261845031-1688872107367-zcFnFYZY70Il3TknE9kbvCZy"
    ...
    for key in pattern_dict.keys():
        ...
        if match:
            ...
            if key == "OBJ0":
                ...
                # store["YES"] = True
                # store["NO"] = False
                tokenizer = Tokenizer(commands_dict['path'])
                # reply = tokenizer.get_frequent_words(int(commands_dict['n_top_words']))
                reply = tokenizer.tokenize()
                break
            elif key == "OBJ2":
